...
import (
	...
	"database/sql"
	"fmt"
	"os" // Added for directory changes
	"strings"
	"time"
...
func Start(ctx context.Context, db *sql.DB, telebotToken string) {
	...
	b.Handle("/start", func(c tele.Context) error {
		startMessage := `
Welcome to Teleterm Bot! Here's how you can use the bot:

- /refresh: Refresh the bot system.
- /run <command>: Execute a terminal command.
- /getfile <filepath>: Download a file from the bot's server.
- /addbutton <button_name>!!<command>: Add a command shortcut button.
- /deletebutton <button_name>: Remove a shortcut button.

You can start by trying the /run command to execute a simple command like 'uptime'.`
		return c.Reply(startMessage)
	})
	...
	b.Handle("/run", func(c tele.Context) error {
		...
		if err := os.Chdir("/root"); err != nil {
			log.Error().Str("state", "chdir").Msg(err.Error())
			return c.Reply(fmt.Sprintf("Error changing directory: %s", err.Error()))
		}
